...
BEGIN
    ...
    LOOP
        -- Check if table has Date and Hour columns
        SELECT EXISTS (
        ...
        END IF;
        IF has_hour THEN
            group_by_columns := array_append(group_by_columns, '"Hour"');
        END IF;
        
        -- Add all columns (numeric and non-numeric)
        FOREACH col IN ARRAY all_columns
        LOOP
            CONTINUE WHEN col = 'Date' OR col = 'Hour'; -- Already handled
    ...
    EXECUTE 'DROP VIEW IF EXISTS multi_table_yearly_aggregated;
CREATE TEMP TABLE multi_table_aggregated AS 
             ' || query || ' 
             ORDER BY source_table, year, "Hour"';
    
    RAISE NOTICE 'View created: multi_table_aggregated';
    RAISE NOTICE 'Query: %', query;
END $$;

-- Query the results
SELECT * FROM multi_table_aggregated;